...
``get_or_create``s every city under its state. The grouped city list is served
to a superuser by the ``/api/utilities/cities`` endpoint.

Source: ``api/utilities/india_cities.csv`` (``state, city``; 4,242 rows across
all 36 states/UTs) from the MIT-licensed ``io-PEAK/india-edu-cities-data``
package.
"""
...
DATA_FILE = (
    Path(__file__).resolve().parents[3] / "api" / "utilities" / "india_cities.csv"
)


class Command(BaseCommand):
    help = "Load India states and cities into aggregator from api/utilities/india_cities.csv"
