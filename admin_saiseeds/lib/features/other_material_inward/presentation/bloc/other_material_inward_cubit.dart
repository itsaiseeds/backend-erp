...
import '../../data/models/paginated_other_material_inward_model.dart';
import '../../data/other_material_inward_repository.dart';

enum OtherMaterialInwardStatus { initial, loading, loaded, failure }

class OtherMaterialInwardState extends Equatable {
  final OtherMaterialInwardStatus status;
  ...
  const OtherMaterialInwardState({
    this.status = OtherMaterialInwardStatus.initial,
    this.lots = const [],
  ...
  OtherMaterialInwardState copyWith({
    ...
    return OtherMaterialInwardState(
...
class OtherMaterialInwardCubit extends SafeCubit<OtherMaterialInwardState> {
  final OtherMaterialInwardRepository _repository;

  OtherMaterialInwardCubit({required OtherMaterialInwardRepository repository})
    : _repository = repository,
      super(const OtherMaterialInwardState());
  ...
  Future<bool> deleteLot(String publicId) =>
      _mutate(() => _repository.deleteLot(publicId));
  ...
  Future<void> _fetch({required int page}) async {
    ...
    try {
      final PaginatedOtherMaterialInwardModel result = await _repository
          .fetchLots(queryParams: _buildQueryParams(page));
      ...
      emit(
        state.copyWith(
          status: OtherMaterialInwardStatus.failure,
